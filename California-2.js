const CAMPAIGN_NAME = 'TFD-CALIFORNIA-PKG';
const AD_GROUP_NAME = 'TFD-CALIFORNIA-ESTANDAR';
const JSON_URL = 'https://www.reservhotel.com/hotel_air_tools/10815.json';

// Iniciales de los aeropuertos
const AIRPORT_IAH = 'SFO';
const AIRPORT_DWF = 'LAX';
...
function main() {
  ...
  const filteredOffers = offers.filter(offer => offer.departure === AIRPORT_IAH || offer.departure === AIRPORT_DWF);

  if (filteredOffers.length > 0) {
    // Ordena las ofertas filtradas por el costo en orden ascendente
    filteredOffers.sort((a, b) => a.cost - b.cost);
    const adText = createAdText(filteredOffers[0]); // Usar la oferta con el costo más bajo
    ...
    if (adGroupIterator.hasNext()) {
      ...
      if (existingAds.length > 0) {
        ...
        if (!isAdUpToDate(existingAd, adText)) {
          ...
          try {
            updateResponsiveSearchAd(existingAd, adText);
            Logger.log('Successfully updated ad for departure: ' + adText.departure);
          } catch (e) {
            Logger.log('Failed to update ad for departure: ' + adText.departure + '. Error: ' + e.message);
          }
        } else {
...
function createAdText(offer) {
  ...
  return {
    ...
    cost: cost,
    headlines: [
      truncateText(`Fly from California to Cancun`, 30),
    ...
    descriptions: [
      truncateText(`Flight from California to Cancun from $${cost}. Book Today!`, 90),
      truncateText(`Enjoy a stay at The Fives Downtown Hotel + Flight from $${cost}`, 90),
...
// Función para actualizar un anuncio responsivo
function updateResponsiveSearchAd(existingAd, adText) {
  const ad = existingAd.ad;
  adText.headlines.forEach((headline, index) => {
    ad['setHeadlinePart' + (index + 1)](headline);
  });
  adText.descriptions.forEach((description, index) => {
    ad['setDescription' + (index + 1)](description);
  });
  ad.urls().setFinalUrl(adText.finalUrl);
}